...
const _defaultConfig = node_core_library_1.JsonFile.load(path.join(__dirname, '../schemas/api-extractor-defaults.json'));
/**
 ...
 **/
const _declarationFileExtensionRegExp = /\.d(\.[^./\\]+)?\.(c|m)?ts$/i;
/**
...
class ExtractorConfig {
     ...
     */
    static jsonSchema = node_core_library_1.JsonSchema.fromLoadedObject(api_extractor_schema_json_1.default);
    /**
     * The config file name "api-extractor.json".
     */
    static FILENAME = 'api-extractor.json';
    /**
     * The full path to `extends/tsdoc-base.json` which contains the standard TSDoc configuration
     * for API Extractor.
     * @internal
     */
    static _tsdocBaseFilePath = path.resolve(__dirname, '../../extends/tsdoc-base.json');
     ...
     */
    get reportFilePath() {
        const completeConfig = this.#getCompleteReportConfig();
    ...
    static loadFile(jsonFilePath) {
        ...
        try {
            do {
                ...
                node_core_library_1.Objects.mergeWith(baseConfig, configObject, mergeCustomizer);
        ...
        configObject = node_core_library_1.Objects.mergeWith(structuredClone(_defaultConfig), configObject, mergeCustomizer);
    ...
    static prepare(options) {
        ...
        try {
            ...
            const mainEntryPointFilePath = _resolvePathWithTokens('mainEntryPointFilePath', configObject.mainEntryPointFilePath, tokenContext);
            ...
            const tsconfigFilePath = _resolvePathWithTokens('tsconfigFilePath', configObject.compiler.tsconfigFilePath, tokenContext);
            ...
            const apiReportEnabled = configObject.apiReport?.enabled ?? false;
            const apiReportIncludeForgottenExports = configObject.apiReport?.includeForgottenExports ?? false;
            ...
            if (apiReportEnabled) {
                ...
                const reportVariantKinds = apiReportConfig.reportVariants ?? defaultApiReportVariants;
                for (const reportVariantKind of reportVariantKinds) {
                    ...
                    const normalizedFileName = _expandStringWithTokens('reportFileName', fileNameWithTokens, tokenContext);
                    reportConfigs.push({
                ...
                if (apiReportConfig.reportTempFolder) {
                    reportTempFolder = _resolvePathWithTokens('reportTempFolder', apiReportConfig.reportTempFolder, tokenContext);
                }
                tagsToReport = {
                    ...defaultTagsToReport,
            ...
            if (configObject.docModel?.enabled) {
                apiJsonFilePath = _resolvePathWithTokens('apiJsonFilePath', configObject.docModel.apiJsonFilePath, tokenContext);
            ...
            if (configObject.tsdocMetadata) {
                ...
                if (tsdocMetadataEnabled) {
                    ...
                    if (tsdocMetadataFilePath.trim() === '<lookup>') {
                        ...
                        tsdocMetadataFilePath = _resolvePathWithTokens('tsdocMetadataFilePath', configObject.tsdocMetadata.tsdocMetadataFilePath, tokenContext);
                    }
            ...
            if (configObject.dtsRollup) {
                ...
                untrimmedFilePath = _resolvePathWithTokens('untrimmedFilePath', configObject.dtsRollup.untrimmedFilePath, tokenContext);
                alphaTrimmedFilePath = _resolvePathWithTokens('alphaTrimmedFilePath', configObject.dtsRollup.alphaTrimmedFilePath, tokenContext);
                betaTrimmedFilePath = _resolvePathWithTokens('betaTrimmedFilePath', configObject.dtsRollup.betaTrimmedFilePath, tokenContext);
                publicTrimmedFilePath = _resolvePathWithTokens('publicTrimmedFilePath', configObject.dtsRollup.publicTrimmedFilePath, tokenContext);
            ...
            const enumMemberOrder = configObject.enumMemberOrder ?? api_extractor_model_1.EnumMemberOrder.ByName;
...
function _resolveConfigFileRelativePaths(configFile, currentConfigFolderPath) {
    if (configFile.projectFolder) {
        configFile.projectFolder = _resolveConfigFileRelativePath('projectFolder', configFile.projectFolder, currentConfigFolderPath);
    }
    if (configFile.mainEntryPointFilePath) {
        configFile.mainEntryPointFilePath = _resolveConfigFileRelativePath('mainEntryPointFilePath', configFile.mainEntryPointFilePath, currentConfigFolderPath);
    }
    if (configFile.compiler) {
        if (configFile.compiler.tsconfigFilePath) {
            configFile.compiler.tsconfigFilePath = _resolveConfigFileRelativePath('tsconfigFilePath', configFile.compiler.tsconfigFilePath, currentConfigFolderPath);
        }
    }
    if (configFile.apiReport) {
        if (configFile.apiReport.reportFolder) {
            configFile.apiReport.reportFolder = _resolveConfigFileRelativePath('reportFolder', configFile.apiReport.reportFolder, currentConfigFolderPath);
        }
        if (configFile.apiReport.reportTempFolder) {
            configFile.apiReport.reportTempFolder = _resolveConfigFileRelativePath('reportTempFolder', configFile.apiReport.reportTempFolder, currentConfigFolderPath);
        }
    }
    if (configFile.docModel) {
        if (configFile.docModel.apiJsonFilePath) {
            configFile.docModel.apiJsonFilePath = _resolveConfigFileRelativePath('apiJsonFilePath', configFile.docModel.apiJsonFilePath, currentConfigFolderPath);
        }
    }
    if (configFile.dtsRollup) {
        if (configFile.dtsRollup.untrimmedFilePath) {
            configFile.dtsRollup.untrimmedFilePath = _resolveConfigFileRelativePath('untrimmedFilePath', configFile.dtsRollup.untrimmedFilePath, currentConfigFolderPath);
        }
        if (configFile.dtsRollup.alphaTrimmedFilePath) {
            configFile.dtsRollup.alphaTrimmedFilePath = _resolveConfigFileRelativePath('alphaTrimmedFilePath', configFile.dtsRollup.alphaTrimmedFilePath, currentConfigFolderPath);
        }
        if (configFile.dtsRollup.betaTrimmedFilePath) {
            configFile.dtsRollup.betaTrimmedFilePath = _resolveConfigFileRelativePath('betaTrimmedFilePath', configFile.dtsRollup.betaTrimmedFilePath, currentConfigFolderPath);
        }
        if (configFile.dtsRollup.publicTrimmedFilePath) {
            configFile.dtsRollup.publicTrimmedFilePath = _resolveConfigFileRelativePath('publicTrimmedFilePath', configFile.dtsRollup.publicTrimmedFilePath, currentConfigFolderPath);
        }
    }
    if (configFile.tsdocMetadata) {
        if (configFile.tsdocMetadata.tsdocMetadataFilePath) {
            configFile.tsdocMetadata.tsdocMetadataFilePath = _resolveConfigFileRelativePath('tsdocMetadataFilePath', configFile.tsdocMetadata.tsdocMetadataFilePath, currentConfigFolderPath);
        }
    }
}
function _resolveConfigFileRelativePath(fieldName, fieldValue, currentConfigFolderPath) {
...
function _resolvePathWithTokens(fieldName, value, tokenContext) {
...
function _expandStringWithTokens(fieldName, value, tokenContext) {
 ...
 */
function _rejectAnyTokensInPath(value, fieldName) {
