...
class Collector {
    ...
    #astEntryPoint;
    #entities = [];
    #entitiesByAstEntity = new Map();
    #entitiesBySymbol = new Map();
    #starExportedExternalModulePaths = [];
    #dtsTypeReferenceDirectives = new Set();
    #dtsLibReferenceDirectives = new Set();
    // Used by getOverloadIndex()
    #cachedOverloadIndexesByDeclaration;
    constructor(options) {
        ...
        const entryPointSourceFile = program.getSourceFile(this.extractorConfig.mainEntryPointFilePath);
        ...
        this.bundledPackageNames = _resolveBundledPackagePatterns(this.extractorConfig.bundledPackages, this.extractorConfig.packageJson);
        ...
        this.#cachedOverloadIndexesByDeclaration = new Map();
    }
    /**a
    ...
    analyze() {
        ...
        node_core_library_1.Sort.sortBy(this.#entities, (x) => x.getSortKey());
        node_core_library_1.Sort.sortSet(this.#dtsTypeReferenceDirectives);
        node_core_library_1.Sort.sortSet(this.#dtsLibReferenceDirectives);
    ...
    getOverloadIndex(astDeclaration) {
        ...
        let overloadIndex = this.#cachedOverloadIndexesByDeclaration.get(astDeclaration);
    ...
    #createCollectorEntity(astEntity, exportName, parent) {
        let entity = this.#entitiesByAstEntity.get(astEntity);
        if (!entity) {
    ...
    #recursivelyCreateEntities(astEntity, alreadySeenAstEntities) {
        ...
        if (astEntity instanceof AstNamespaceImport_1.AstNamespaceImport) {
            ...
            const parentEntity = this.#entitiesByAstEntity.get(astEntity);
            if (!parentEntity) {
    ...
    #calculateDeclarationMetadataForDeclarations(astSymbol) {
    ...
    #addAncillaryDeclaration(mainAstDeclaration, ancillaryAstDeclaration) {
    ...
    #calculateApiItemMetadata(astDeclaration) {
        ...
        if (options.effectiveReleaseTag === api_extractor_model_1.ReleaseTag.None) {
            if (!astDeclaration.astSymbol.isExternal) {
                ...
                const entity = this.#entitiesByAstEntity.get(astSymbol.rootAstSymbol);
                if (entity &&
                    (entity.consumable ||
    ...
    #parseTsdocForAstDeclaration(astDeclaration) {
        ...
        const parserContext = this.#tsdocParser.parseRange(tsdocTextRange);
    ...
    #collectReferenceDirectives(astEntity) {
    ...
    #collectReferenceDirectivesFromSourceFiles(sourceFiles, onlyIncludeExplicitlyPreserved) {
            ...
            if (sourceFile?.fileName) {
                ...
                if (!seenFilenames.has(fileName)) {
                    ...
                    for (const typeReferenceDirective of typeReferenceDirectives) {
                        const name = this.#getReferenceDirectiveFromSourceFile(sourceFileText, typeReferenceDirective, onlyIncludeExplicitlyPreserved);
                        if (name) {
                            this.#dtsTypeReferenceDirectives.add(name);
                        }
                    }
                    for (const libReferenceDirective of libReferenceDirectives) {
                        const reference = this.#getReferenceDirectiveFromSourceFile(sourceFileText, libReferenceDirective, onlyIncludeExplicitlyPreserved);
                        if (reference) {
    ...
    #getReferenceDirectiveFromSourceFile(sourceFileText, { pos, end, preserve }, onlyIncludeExplicitlyPreserved) {
 ...
 */
function _resolveBundledPackagePatterns(bundledPackages, packageJson) {
