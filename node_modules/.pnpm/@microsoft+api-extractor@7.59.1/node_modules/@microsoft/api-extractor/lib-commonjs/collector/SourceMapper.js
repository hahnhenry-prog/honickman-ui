...
class SourceMapper {
    ...
    getSourceLocation(options) {
        ...
        const mappedSourceLocation = this.#getMappedSourceLocation(sourceLocation);
        return mappedSourceLocation || sourceLocation;
    }
    #getMappedSourceLocation(sourceLocation) {
        ...
        const sourceMap = this.#getSourceMap(sourceFilePath);
        if (!sourceMap)
            return;
        const nearestMappingItem = _findNearestMappingItem(sourceMap.mappingItems, {
            line: sourceFileLine,
        ...
        let originalFileInfo = this.#originalFileInfoByPath.get(mappedFilePath);
    ...
    #getSourceMap(sourceFilePath) {
        let sourceMap = this.#sourceMapByFilePath.get(sourceFilePath);
...
function _findNearestMappingItem(mappingItems, position) {
    ...
    while (startIndex <= endIndex) {
        ...
        const diff = _compareMappingItem(mappingItems[middleIndex], position);
        if (diff < 0) {
...
function _compareMappingItem(mappingItem, position) {
