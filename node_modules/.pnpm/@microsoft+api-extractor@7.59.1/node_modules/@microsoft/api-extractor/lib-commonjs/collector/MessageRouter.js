...
class MessageRouter {
    ...
    showDiagnostics;
    constructor(options) {
        const { workingPackageFolder, messageCallback, sourceMapper, tsdocConfiguration, showVerboseMessages, showDiagnostics, messagesConfig } = options;
        this.#workingPackageFolder = workingPackageFolder;
        this.#messageCallback = messageCallback;
        this.#messages = [];
        this.#associatedMessagesForAstDeclaration = new Map();
     ...
     */
    #applyMessagesConfig(messagesConfig) {
        if (messagesConfig.compilerMessageReporting) {
            for (const messageId of Object.getOwnPropertyNames(messagesConfig.compilerMessageReporting)) {
                const reportingRule = _getNormalizedRule(messagesConfig.compilerMessageReporting[messageId]);
        ...
        if (messagesConfig.extractorMessageReporting) {
            for (const messageId of Object.getOwnPropertyNames(messagesConfig.extractorMessageReporting)) {
                const reportingRule = _getNormalizedRule(messagesConfig.extractorMessageReporting[messageId]);
                if (messageId === 'default') {
        ...
        if (messagesConfig.tsdocMessageReporting) {
            for (const messageId of Object.getOwnPropertyNames(messagesConfig.tsdocMessageReporting)) {
                const reportingRule = _getNormalizedRule(messagesConfig.tsdocMessageReporting[messageId]);
                if (messageId === 'default') {
    ...
    addCompilerDiagnostic(diagnostic) {
        ...
        this.#messages.push(new ExtractorMessage_1.ExtractorMessage(options));
    }
    /**
    ...
    #associateMessageWithAstDeclaration(extractorMessage, astDeclaration) {
        let associatedMessages = this.#associatedMessagesForAstDeclaration.get(astDeclaration);
    ...
    fetchAssociatedMessagesForReviewFile(astDeclaration) {
        ...
        const associatedMessages = this.#associatedMessagesForAstDeclaration.get(astDeclaration) || [];
        for (const associatedMessage of associatedMessages) {
            ...
            if (!associatedMessage.handled) {
                ...
                const reportingRule = this.#getRuleForMessage(associatedMessage);
    ...
    fetchUnassociatedMessagesForReviewFile() {
        ...
        for (const unassociatedMessage of this.messages) {
            ...
            if (!unassociatedMessage.handled) {
                // Is this message type configured to go in the API report file?
                const reportingRule = this.#getRuleForMessage(unassociatedMessage);
                if (reportingRule.addToApiReportFile) {
                    // Include it in the result, and record that it went to the API report file
    ...
    logError(messageId, message, properties) {
        this.#handleMessage(new ExtractorMessage_1.ExtractorMessage({
    ...
    #handleMessage(message) {
        ...
        if (message.category === ExtractorMessage_1.ExtractorMessageCategory.Console) {
            ...
            const reportingRule = this.#getRuleForMessage(message);
    ...
    #getRuleForMessage(message) {
        const reportingRule = this.#reportingRuleByMessageId.get(message.messageId);
        if (reportingRule) {
     ...
     */
    #sortMessagesForOutput(messages) {
        messages.sort((a, b) => {
            let diff;
...
function _getNormalizedRule(rule) {
    return {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function _buildJsonDumpObject(input, keyNamesToOmit) {
        ...
        case 'object':
            if (Array.isArray(input)) {
                ...
                for (const element of input) {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const serializedElement = _buildJsonDumpObject(element, keyNamesToOmit);
            ...
            for (const key of Object.getOwnPropertyNames(input)) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const serializedValue = _buildJsonDumpObject(value, keyNamesToOmit);
