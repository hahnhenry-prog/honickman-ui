...
class DeclarationReferenceGenerator {
    ...
    getDeclarationReferenceForIdentifier(node) {
        const symbol = this.#collector.typeChecker.getSymbolAtLocation(node);
        if (symbol !== undefined) {
            const isExpression = _isInExpressionContext(node);
    ...
    #getNavigationToSymbol(symbol) {
        ...
        const isFromExternalLibrary = !!sourceFile && this.#collector.program.isSourceFileFromExternalLibrary(sourceFile);
        ...
        const entity = this.#collector.tryGetEntityForSymbol(symbol);
    ...
    #symbolToDeclarationReference(symbol, meaning, includeModuleSymbols) {
        const declaration = TypeScriptHelpers_1.TypeScriptHelpers.tryGetADeclaration(symbol);
        const sourceFile = declaration?.getSourceFile();
        ...
        if (_isExternalModuleSymbol(followedSymbol)) {
            ...
            return new DeclarationReference_1.DeclarationReference(this.#sourceFileToModuleSource(sourceFile));
        }
        // Do not generate a declaration reference for a type parameter.
        if (followedSymbol.flags & ts.SymbolFlags.TypeParameter) {
            return undefined;
        }
        let parentRef = this.#getParentReference(followedSymbol);
        if (!parentRef) {
            return undefined;
        }
        let localName = followedSymbol.name;
        const entity = this.#collector.tryGetEntityForSymbol(followedSymbol);
        ...
        const navigation = this.#getNavigationToSymbol(followedSymbol);
    ...
    #getParentReference(symbol) {
        ...
        if (entity) {
            ...
            if (firstExportingConsumableParent &&
                ...
                const parentSymbol = TypeScriptInternals_1.TypeScriptInternals.tryGetSymbolForDeclaration(firstExportingConsumableParent.astEntity.declaration, this.#collector.typeChecker);
                if (parentSymbol) {
                    return this.#symbolToDeclarationReference(parentSymbol, parentSymbol.flags, 
        ...
        if (grandParent && ts.isModuleDeclaration(grandParent)) {
            const grandParentSymbol = TypeScriptInternals_1.TypeScriptInternals.tryGetSymbolForDeclaration(grandParent, this.#collector.typeChecker);
            if (grandParentSymbol) {
                return this.#symbolToDeclarationReference(grandParentSymbol, grandParentSymbol.flags, 
                /*includeModuleSymbols*/ true);
            }
        }
    ...
    #getPackageName(sourceFile) {
        if (this.#collector.program.isSourceFileFromExternalLibrary(sourceFile)) {
            const packageJson = this.#collector.packageJsonLookup.tryLoadNodePackageJsonFor(sourceFile.fileName);
            if (packageJson && packageJson.name) {
                return packageJson.name;
            }
            return _a.unknownReference;
        }
        return this.#collector.workingPackage.name;
    }
    #sourceFileToModuleSource(sourceFile) {
        if (sourceFile && ts.isExternalModule(sourceFile)) {
            ...
            if (this.#collector.bundledPackageNames.has(packageName)) {
                ...
                return new DeclarationReference_1.ModuleSource(this.#collector.workingPackage.name);
            }
            else {
...
_a = DeclarationReferenceGenerator;
function _isInExpressionContext(node) {
...
function _isExternalModuleSymbol(symbol) {
    return (!!(symbol.flags & ts.SymbolFlags.ValueModule) &&
        symbol.valueDeclaration !== undefined &&
        ts.isSourceFile(symbol.valueDeclaration));
}
function _isSameSymbol(left, right) {
    return (left === right ||
        !!(left &&
...
function _getMeaningOfSymbol(symbol, meaning) {
