...
class AstSymbolTable {
     ...
     */
    #astSymbolsBySymbol = new Map();
    /**
     * A mapping from ts.Declaration --> AstDeclaration
     */
    #astDeclarationsByDeclaration = new Map();
    // Note that this is a mapping from specific AST nodes that we analyzed, based on the underlying symbol
    // for that node.
    #entitiesByNode = new Map();
    constructor(program, typeChecker, packageJsonLookup, bundledPackageNames, messageRouter) {
        this.#program = program;
        this.#typeChecker = typeChecker;
        this.#messageRouter = messageRouter;
        this.#globalVariableAnalyzer = TypeScriptInternals_1.TypeScriptInternals.getGlobalVariableAnalyzer(program);
        this.#packageMetadataManager = new PackageMetadataManager_1.PackageMetadataManager(packageJsonLookup, messageRouter);
        this.#exportAnalyzer = new ExportAnalyzer_1.ExportAnalyzer(this.#program, this.#typeChecker, bundledPackageNames, {
            analyze: this.analyze.bind(this),
            fetchAstSymbol: this.#fetchAstSymbol.bind(this)
        });
        this.#alreadyWarnedGlobalNames = new Set();
    }
    /**
    ...
    getChildAstDeclarationByNode(node, parentAstDeclaration) {
        ...
        const childAstDeclaration = this.#astDeclarationsByDeclaration.get(node);
    ...
    #analyzeAstNamespaceImport(astNamespaceImport) {
     ...
     */
    #analyzeChildTree(node, governingAstDeclaration) {
        switch (node.kind) {
            ...
            case ts.SyntaxKind.TypeQuery: // represents for "typeof X" as a type
                {
                    ...
                    if (identifierNode) {
                        let referencedAstEntity = this.#entitiesByNode.get(identifierNode);
                        if (!referencedAstEntity) {
                            const symbol = this.#typeChecker.getSymbolAtLocation(identifierNode);
                            if (!symbol) {
                            ...
                            if (displacedSymbol) {
                                if (this.#globalVariableAnalyzer.hasGlobalName(identifierNode.text)) {
                                    ...
                                    if (this.#messageRouter.showDiagnostics) {
                                        if (!this.#alreadyWarnedGlobalNames.has(identifierNode.text)) {
                                            ...
                                            this.#messageRouter.logDiagnostic(`Ignoring reference to global variable "${identifierNode.text}"` +
                                                ` in ` +
                                ...
                                referencedAstEntity = this.#exportAnalyzer.fetchReferencedAstEntity(symbol, governingAstDeclaration.astSymbol.isExternal);
            ...
            case ts.SyntaxKind.ImportType:
                {
                    ...
                    let referencedAstEntity = this.#entitiesByNode.get(importTypeNode);
        ...
        const newGoverningAstDeclaration = this.#fetchAstDeclaration(node, governingAstDeclaration.astSymbol.isExternal);
        for (const childNode of node.getChildren()) {
            this.#analyzeChildTree(childNode, newGoverningAstDeclaration || governingAstDeclaration);
        }
    }
    #fetchEntityForNode(node, governingAstDeclaration) {
        let referencedAstEntity = this.#entitiesByNode.get(node);
        if (!referencedAstEntity) {
            if (node.kind === ts.SyntaxKind.ImportType) {
                referencedAstEntity = this.#exportAnalyzer.fetchReferencedAstEntityFromImportTypeNode(node, governingAstDeclaration.astSymbol.isExternal);
            }
            else {
    ...
    #fetchAstDeclaration(node, isExternal) {
        if (!AstDeclaration_1.AstDeclaration.isSupportedSyntaxKind(node.kind)) {
            return undefined;
        }
        const symbol = TypeScriptHelpers_1.TypeScriptHelpers.getSymbolForDeclaration(node, this.#typeChecker);
        if (!symbol) {
            throw new node_core_library_1.InternalError('Unable to find symbol for node');
        }
        const astSymbol = this.#fetchAstSymbol({
    ...
    #fetchAstSymbol(options) {
        ...
        if (TypeScriptHelpers_1.TypeScriptHelpers.isAmbient(followedSymbol, this.#typeChecker)) {
        ...
        if (TypeScriptHelpers_1.TypeScriptHelpers.isFollowableAlias(followedSymbol, this.#typeChecker)) {
        ...
        let astSymbol = this.#astSymbolsBySymbol.get(followedSymbol);
        if (!astSymbol) {
            ...
            if (!nominalAnalysis) {
                ...
                if (arbitraryDeclaration) {
                    const arbitraryParentDeclaration = this.#tryFindFirstAstDeclarationParent(arbitraryDeclaration);
                    if (arbitraryParentDeclaration) {
                        const parentSymbol = TypeScriptHelpers_1.TypeScriptHelpers.getSymbolForDeclaration(arbitraryParentDeclaration, this.#typeChecker);
            ...
            const localName = options.localName || _a.getLocalNameForSymbol(followedSymbol);
            ...
            for (const declaration of followedSymbol.declarations || []) {
                ...
                if (parentAstSymbol) {
                    const parentDeclaration = this.#tryFindFirstAstDeclarationParent(declaration);
     ...
     */
    #tryFindFirstAstDeclarationParent(node) {
