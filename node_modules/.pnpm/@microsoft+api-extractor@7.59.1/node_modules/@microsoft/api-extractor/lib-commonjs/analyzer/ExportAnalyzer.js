...
class ExportAnalyzer {
    ...
    #bundledPackageNames;
    #astSymbolTable;
    #astModulesByModuleSymbol = new Map();
    // Used with isImportableAmbientSourceFile()
    #importableAmbientSourceFiles = new Set();
    #astImportsByKey = new Map();
    #astNamespaceImportByModule = new Map();
    constructor(program, typeChecker, bundledPackageNames, astSymbolTable) {
        this.#program = program;
        this.#typeChecker = typeChecker;
        this.#bundledPackageNames = bundledPackageNames;
        this.#astSymbolTable = astSymbolTable;
    }
    /**
    ...
    fetchAstModuleFromSourceFile(sourceFile, moduleReference, isExternal) {
        const moduleSymbol = this.#getModuleSymbolFromSourceFile(sourceFile, moduleReference);
        ...
        let astModule = this.#astModulesByModuleSymbol.get(moduleSymbol);
        if (!astModule) {
            ...
            this.#astModulesByModuleSymbol.set(moduleSymbol, astModule);
            if (astModule.isExternal) {
                ...
                for (const exportedSymbol of this.#typeChecker.getExportsOfModule(moduleSymbol)) {
                    ...
                    const followedSymbol = TypeScriptHelpers_1.TypeScriptHelpers.followAliases(exportedSymbol, this.#typeChecker);
                    ...
                    if (arbitraryDeclaration) {
                        const astSymbol = this.#astSymbolTable.fetchAstSymbol({
                    ...
                    if (exportStarSymbol) {
                        for (const exportStarDeclaration of exportStarSymbol.getDeclarations() || []) {
                            if (ts.isExportDeclaration(exportStarDeclaration)) {
                                const starExportedModule = this.#fetchSpecifierAstModule(exportStarDeclaration, exportStarSymbol);
     ...
     */
    #getModuleSymbolFromSourceFile(sourceFile, moduleReference) {
        const moduleSymbol = TypeScriptInternals_1.TypeScriptInternals.tryGetSymbolForDeclaration(sourceFile, this.#typeChecker);
        ...
        if (moduleReference !== undefined) {
            ...
            if ((moduleReference.moduleSpecifierSymbol.flags & ts.SymbolFlags.Alias) !== 0) {
                ...
                let followedSymbol = TypeScriptInternals_1.TypeScriptInternals.getImmediateAliasedSymbol(moduleReference.moduleSpecifierSymbol, this.#typeChecker);
                if (followedSymbol === undefined) {
                    ...
                    followedSymbol = this.#typeChecker.getAliasedSymbol(moduleReference.moduleSpecifierSymbol);
                }
                if (followedSymbol !== undefined && followedSymbol !== moduleReference.moduleSpecifierSymbol) {
                    ...
                    if (parent !== undefined) {
                        ...
                        if ((parent.flags & ts.SymbolFlags.ValueModule) !== 0) {
                            ...
                            this.#importableAmbientSourceFiles.add(sourceFile);
                            return parent;
                        }
                    }
                }
    ...
    fetchAstModuleExportInfo(entryPointAstModule) {
        ...
        if (entryPointAstModule.astModuleExportInfo === undefined) {
            ...
            this.#collectAllExportsRecursive(astModuleExportInfo, entryPointAstModule);
     ...
     */
    #isExternalModulePath(importOrExportDeclaration, moduleSpecifier) {
        ...
        const mode = specifier && ts.isStringLiteralLike(specifier)
            ? TypeScriptInternals_1.TypeScriptInternals.getModeForUsageLocation(importOrExportDeclaration.getSourceFile(), specifier, this.#program.getCompilerOptions())
            : undefined;
        const resolvedModule = TypeScriptInternals_1.TypeScriptInternals.getResolvedModule(this.#program, importOrExportDeclaration.getSourceFile(), moduleSpecifier, mode);
        ...
        if (packageName !== undefined && this.#bundledPackageNames.has(packageName)) {
            return false;
        }
    ...
    isImportableAmbientSourceFile(sourceFile) {
        return this.#importableAmbientSourceFiles.has(sourceFile);
    }
    #collectAllExportsRecursive(astModuleExportInfo, astModule) {
        ...
        if (astModule.isExternal) {
                        ...
                        default:
                            ...
                            if (exportName !== ts.InternalSymbolName.Default || visitedAstModules.size === 1) {
                                if (!exportedLocalEntities.has(exportSymbol.name)) {
                                    const astEntity = this.#getExportOfAstModule(exportSymbol.name, astModule);
                                    if (astEntity instanceof AstSymbol_1.AstSymbol && !astEntity.isExternal) {
                                        this.#astSymbolTable.analyze(astEntity);
                                    }
            ...
            for (const starExportedModule of astModule.starExportedModules) {
                this.#collectAllExportsRecursive(astModuleExportInfo, starExportedModule);
            }
        }
    }
    /**
    ...
    fetchReferencedAstEntity(symbol, referringModuleIsExternal) {
        ...
        if (referringModuleIsExternal) {
            current = TypeScriptHelpers_1.TypeScriptHelpers.followAliases(symbol, this.#typeChecker);
        }
        else {
            for (;;) {
                ...
                for (const declaration of current.declarations || []) {
                    ...
                    matchedAstEntity = this.#tryMatchExportDeclaration(declaration, current);
                    if (matchedAstEntity !== undefined) {
                        return matchedAstEntity;
                    }
                    matchedAstEntity = this.#tryMatchImportDeclaration(declaration, current);
                    if (matchedAstEntity !== undefined) {
                        return matchedAstEntity;
                    }
                }
                ...
                const currentAlias = TypeScriptInternals_1.TypeScriptInternals.getImmediateAliasedSymbol(current, this.#typeChecker);
    ...
    fetchReferencedAstEntityFromImportTypeNode(node, referringModuleIsExternal) {
        const externalModulePath = this.#tryGetExternalModulePath(node);
        if (externalModulePath) {
            ...
            return this.#fetchAstImport(undefined, {
        ...
        const exportSymbol = this.#typeChecker.getSymbolAtLocation(rightMostToken);
        if (!exportSymbol) {
        ...
        for (;;) {
            ...
            const currentAlias = this.#typeChecker.getAliasedSymbol(followedSymbol);
    ...
    #tryMatchExportDeclaration(declaration, declarationSymbol) {
        ...
        if (exportDeclaration) {
            ...
            if (declaration.kind === ts.SyntaxKind.ExportSpecifier) {
                ...
                const astModule = this.#fetchSpecifierAstModule(exportDeclaration, declarationSymbol);
                return this.#getAstNamespaceExport(astModule, declarationSymbol, declaration);
            }
            else {
            ...
            if (exportDeclaration.moduleSpecifier) {
                const externalModulePath = this.#tryGetExternalModulePath(exportDeclaration);
                if (externalModulePath !== undefined) {
                    return this.#fetchAstImport(declarationSymbol, {
                ...
                return this.#getExportOfSpecifierAstModule(exportName, exportDeclaration, declarationSymbol);
            }
        }
        return undefined;
    }
    #getAstNamespaceExport(astModule, declarationSymbol, declaration) {
        const imoprtNamespace = this.#getAstNamespaceImport(astModule, declarationSymbol, declaration);
    ...
    #tryMatchImportDeclaration(declaration, declarationSymbol) {
        ...
        if (importDeclaration) {
            const externalModulePath = this.#tryGetExternalModulePath(importDeclaration);
            if (declaration.kind === ts.SyntaxKind.NamespaceImport) {
                ...
                //   SemicolonToken:  pre=[;]
                if (externalModulePath === undefined) {
                    const astModule = this.#fetchSpecifierAstModule(importDeclaration, declarationSymbol);
                    return this.#getAstNamespaceImport(astModule, declarationSymbol, declaration);
                }
            ...
            if (declaration.kind === ts.SyntaxKind.ImportSpecifier) {
                ...
                return this.#getExportOfSpecifierAstModule(exportName, importDeclaration, declarationSymbol);
            }
                ...
                return this.#getExportOfSpecifierAstModule(ts.InternalSymbolName.Default, importDeclaration, declarationSymbol);
            }
            else {
    ...
    #getAstNamespaceImport(astModule, declarationSymbol, declaration) {
        let namespaceImport = this.#astNamespaceImportByModule.get(astModule);
        if (namespaceImport === undefined) {
            ...
            this.#astNamespaceImportByModule.set(astModule, namespaceImport);
        }
        return namespaceImport;
    }
    #getExportOfSpecifierAstModule(exportName, importOrExportDeclaration, exportSymbol) {
        const specifierAstModule = this.#fetchSpecifierAstModule(importOrExportDeclaration, exportSymbol);
        const astEntity = this.#getExportOfAstModule(exportName, specifierAstModule);
        return astEntity;
    }
    #getExportOfAstModule(exportName, astModule) {
        const visitedAstModules = new Set();
        const astEntity = this.#tryGetExportOfAstModule(exportName, astModule, visitedAstModules);
    ...
    tryGetExportOfAstModule(exportName, astModule) {
        const visitedAstModules = new Set();
        return this.#tryGetExportOfAstModule(exportName, astModule, visitedAstModules);
    }
    #tryGetExportOfAstModule(exportName, astModule, visitedAstModules) {
        if (visitedAstModules.has(astModule)) {
            return undefined;
        }
        visitedAstModules.add(astModule);
        ...
        for (const starExportedModule of astModule.starExportedModules) {
            astEntity = this.#tryGetExportOfAstModule(exportName, starExportedModule, visitedAstModules);
            if (astEntity !== undefined) {
    ...
    #tryGetExternalModulePath(importOrExportDeclaration) {
        const moduleSpecifier = this.#getModuleSpecifier(importOrExportDeclaration);
        if (this.#isExternalModulePath(importOrExportDeclaration, moduleSpecifier)) {
     ...
     */
    #fetchSpecifierAstModule(importOrExportDeclaration, exportSymbol) {
        const moduleSpecifier = this.#getModuleSpecifier(importOrExportDeclaration);
        const mode = importOrExportDeclaration.moduleSpecifier &&
            ts.isStringLiteralLike(importOrExportDeclaration.moduleSpecifier)
            ? TypeScriptInternals_1.TypeScriptInternals.getModeForUsageLocation(importOrExportDeclaration.getSourceFile(), importOrExportDeclaration.moduleSpecifier, this.#program.getCompilerOptions())
            : undefined;
        const resolvedModule = TypeScriptInternals_1.TypeScriptInternals.getResolvedModule(this.#program, importOrExportDeclaration.getSourceFile(), moduleSpecifier, mode);
        if (resolvedModule === undefined) {
        ...
        const moduleSourceFile = this.#program.getSourceFile(resolvedModule.resolvedFileName);
        if (!moduleSourceFile) {
        ...
        const isExternal = this.#isExternalModulePath(importOrExportDeclaration, moduleSpecifier);
    ...
    #fetchAstImport(importSymbol, options) {
        const key = AstImport_1.AstImport.getKey(options);
        let astImport = this.#astImportsByKey.get(key);
        if (!astImport) {
            astImport = new AstImport_1.AstImport(options);
            this.#astImportsByKey.set(key, astImport);
            if (importSymbol) {
                const followedSymbol = TypeScriptHelpers_1.TypeScriptHelpers.followAliases(importSymbol, this.#typeChecker);
                astImport.astSymbol = this.#astSymbolTable.fetchAstSymbol({
                    followedSymbol: followedSymbol,
                    isExternal: true,
                    includeNominalAnalysis: false,
                    addIfMissing: true
    ...
    #getModuleSpecifier(importOrExportDeclaration) {
...
function _getIsTypeOnly(importDeclaration) {
