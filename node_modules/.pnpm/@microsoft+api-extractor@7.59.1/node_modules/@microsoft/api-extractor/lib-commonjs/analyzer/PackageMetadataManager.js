        ...
        case 'object': {
            if (Array.isArray(exports)) {
                ...
                switch (typeof rootExport) {
                    ...
                    case 'object': {
                        let typesExport = rootExport?.types;
                        while (typesExport) {
    ...
    if (typesVersions) {
        ...
        for (const [version, paths] of Object.entries(typesVersions)) {
            ...
            if (minimumMatchingSemver &&
                ...
                const pathEntry = paths['.'] ?? paths['*'];
                const firstPath = pathEntry?.[0];
                if (firstPath) {
...
function _tryResolveTsdocMetadataFromTypesOrTypingsFields({ typings, types }) {
    const typesField = types ?? typings;
    if (typesField) {
 ...
 */
function _resolveTsdocMetadataPathFromPackageJson(packageFolder, packageJson) {
    const tsdocMetadataRelativePath = _tryResolveTsdocMetadataFromTsdocMetadataField(packageJson) ??
...
class PackageMetadataManager {
    static tsdocMetadataFilename = TSDOC_METADATA_FILENAME;
    #packageJsonLookup;
    #messageRouter;
    #packageMetadataByPackageJsonPath = new Map();
    ...
    tryFetchPackageMetadata(sourceFilePath) {
        const packageJsonFilePath = this.#packageJsonLookup.tryGetPackageJsonFilePathFor(sourceFilePath);
        if (!packageJsonFilePath) {
            return undefined;
        }
        let packageMetadata = this.#packageMetadataByPackageJsonPath.get(packageJsonFilePath);
        if (!packageMetadata) {
            const packageJson = this.#packageJsonLookup.loadNodePackageJson(packageJsonFilePath);
            const packageJsonFolder = node_path_1.default.dirname(packageJsonFilePath);
            let aedocSupported = false;
            const tsdocMetadataPath = _resolveTsdocMetadataPathFromPackageJson(packageJsonFolder, packageJson);
            if (node_core_library_1.FileSystem.exists(tsdocMetadataPath)) {
                this.#messageRouter.logVerbose(ConsoleMessageId_1.ConsoleMessageId.FoundTSDocMetadata, 'Found metadata in ' + tsdocMetadataPath);
