...
class RunAction extends ts_command_line_1.CommandLineAction {
    ...
    async onExecuteAsync() {
        ...
        let configFilename;
        let typescriptCompilerFolder = this.#typescriptCompilerFolderParameter.value;
        if (typescriptCompilerFolder) {
            ...
            if (node_core_library_1.FileSystem.exists(typescriptCompilerFolder)) {
                ...
                if (!typescriptCompilerPackageJson) {
                    throw new Error(`The path specified in the ${this.#typescriptCompilerFolderParameter.longName} parameter is not a package.`);
                }
                else if (typescriptCompilerPackageJson.name !== 'typescript') {
                    throw new Error(`The path specified in the ${this.#typescriptCompilerFolderParameter.longName} parameter is not a TypeScript` +
                        ' compiler package.');
                }
            }
            else {
                throw new Error(`The path specified in the ${this.#typescriptCompilerFolderParameter.longName} parameter does not exist.`);
            }
        }
        let extractorConfig;
