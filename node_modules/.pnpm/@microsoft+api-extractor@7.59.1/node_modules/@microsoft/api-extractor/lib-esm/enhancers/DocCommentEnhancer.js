...
export class DocCommentEnhancer {
    ...
    #analyzeApiItem(astDeclaration) {
        const metadata = this.#collector.fetchApiItemMetadata(astDeclaration);
        ...
        if (metadata.docCommentEnhancerVisitorState === VisitorState.Visiting) {
            this.#collector.messageRouter.addAnalyzerIssue(ExtractorMessageId.CyclicInheritDoc, `The @inheritDoc tag for "${astDeclaration.astSymbol.localName}" refers to its own declaration`, astDeclaration);
            return;
        }
    ...
    #analyzeNeedsDocumentation(astDeclaration, metadata) {
        if (astDeclaration.declaration.kind === ts.SyntaxKind.Constructor) {
            ...
            const configuration = this.#collector.extractorConfig.tsdocConfiguration;
            ...
            if (apiItemMetadata.effectiveReleaseTag === ReleaseTag.Internal) {
                ...
                const classMetadata = this.#collector.fetchApiItemMetadata(classDeclaration);
            ...
            if (metadata.tsdocComment) {
                if (tsdoc.PlainTextEmitter.hasAnyTextContent(metadata.tsdocComment.summarySection, 10)) {
                    ...
                    if (this.#refersToDeclarationInWorkingPackage(metadata.tsdocComment.inheritDocTag.declarationReference)) {
    ...
    #checkForBrokenLinks(astDeclaration, metadata) {
        if (!metadata.tsdocComment) {
            return;
        }
        this.#checkForBrokenLinksRecursive(astDeclaration, metadata.tsdocComment);
    }
    #checkForBrokenLinksRecursive(astDeclaration, node) {
        if (node instanceof tsdoc.DocLinkTag) {
            if (node.codeDestination) {
                ...
                if (this.#refersToDeclarationInWorkingPackage(node.codeDestination)) {
                    const referencedAstDeclaration = this.#collector.astReferenceResolver.resolve(node.codeDestination);
                    if (referencedAstDeclaration instanceof ResolverFailure) {
                        this.#collector.messageRouter.addAnalyzerIssue(ExtractorMessageId.UnresolvedLink, 'The @link reference could not be resolved: ' + referencedAstDeclaration.reason, astDeclaration);
                    }
                }
            }
        }
     ...
     */
    #applyInheritDoc(astDeclaration, docComment, inheritDocTag) {
        if (!inheritDocTag.declarationReference) {
            this.#collector.messageRouter.addAnalyzerIssue(ExtractorMessageId.UnresolvedInheritDocBase, 'The @inheritDoc tag needs a TSDoc declaration reference; signature matching is not supported yet', astDeclaration);
            return;
        }
        ...
        const referencedAstDeclaration = this.#collector.astReferenceResolver.resolve(inheritDocTag.declarationReference);
        if (referencedAstDeclaration instanceof ResolverFailure) {
            this.#collector.messageRouter.addAnalyzerIssue(ExtractorMessageId.UnresolvedInheritDocReference, 'The @inheritDoc reference could not be resolved: ' + referencedAstDeclaration.reason, astDeclaration);
            return;
        }
        this.#analyzeApiItem(referencedAstDeclaration);
        const referencedMetadata = this.#collector.fetchApiItemMetadata(referencedAstDeclaration);
     ...
     */
    #copyInheritedDocs(targetDocComment, sourceDocComment) {
     ...
     */
    #refersToDeclarationInWorkingPackage(declarationReference) {
        return (declarationReference?.packageName === undefined ||
