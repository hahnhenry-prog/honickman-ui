...
export class AstReferenceResolver {
    ...
    resolve(declarationReference) {
        ...
        const astModule = this.#astSymbolTable.fetchAstModuleFromWorkingPackage(this.#workingPackage.entryPointSourceFile);
        ...
        const exportName = this.#getMemberReferenceIdentifier(rootMemberReference);
        if (exportName instanceof ResolverFailure) {
            return exportName;
        }
        const rootAstEntity = this.#astSymbolTable.tryGetExportOfAstModule(exportName, astModule);
        if (rootAstEntity === undefined) {
            return new ResolverFailure(`The package "${this.#workingPackage.name}" does not have an export "${exportName}"`);
        }
        ...
        let currentDeclaration = this.#selectDeclaration(rootAstEntity.astDeclarations, rootMemberReference, rootAstEntity.localName);
        ...
        for (let index = 1; index < declarationReference.memberReferences.length; ++index) {
            ...
            const memberName = this.#getMemberReferenceIdentifier(memberReference);
            ...
            const selectedDeclaration = this.#selectDeclaration(matchingChildren, memberReference, memberName);
    ...
    #getMemberReferenceIdentifier(memberReference) {
    ...
    #selectDeclaration(astDeclarations, memberReference, astSymbolName) {
        ...
        if (memberSelector === undefined) {
            if (astDeclarations.length === 1) {
                ...
                const nonAncillaryMatch = this.#tryDisambiguateAncillaryMatches(astDeclarations);
        ...
        switch (memberSelector.selectorKind) {
            case tsdoc.SelectorKind.System:
                return this.#selectUsingSystemSelector(astDeclarations, memberSelector, astSymbolName);
            case tsdoc.SelectorKind.Index:
                return this.#selectUsingIndexSelector(astDeclarations, memberSelector, astSymbolName);
        }
        return new ResolverFailure(`The selector "${memberSelector.selector}" is not a supported selector type`);
    }
    #selectUsingSystemSelector(astDeclarations, memberSelector, astSymbolName) {
        ...
        if (matches.length > 1) {
            // If we found multiple matches, but the extra ones are all ancillary declarations,
            // then return the main declaration.
            const nonAncillaryMatch = this.#tryDisambiguateAncillaryMatches(matches);
            if (nonAncillaryMatch) {
                return nonAncillaryMatch;
            }
    ...
    #selectUsingIndexSelector(astDeclarations, memberSelector, astSymbolName) {
     ...
     */
    #tryDisambiguateAncillaryMatches(matches) {
        let result = undefined;
        for (const match of matches) {
            const declarationMetadata = this.#collector.fetchDeclarationMetadata(match);
