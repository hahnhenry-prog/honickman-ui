...
export class ExportAnalyzer {
        ...
        if (!astModule) {
            ...
            if (astModule.isExternal) {
                ...
                for (const exportedSymbol of this.#typeChecker.getExportsOfModule(moduleSymbol)) {
                    ...
                    const followedSymbol = TypeScriptHelpers.followAliases(exportedSymbol, this.#typeChecker);
    ...
    #getModuleSymbolFromSourceFile(sourceFile, moduleReference) {
        const moduleSymbol = TypeScriptInternals.tryGetSymbolForDeclaration(sourceFile, this.#typeChecker);
        ...
        if (moduleReference !== undefined) {
            ...
            if ((moduleReference.moduleSpecifierSymbol.flags & ts.SymbolFlags.Alias) !== 0) {
                ...
                let followedSymbol = TypeScriptInternals.getImmediateAliasedSymbol(moduleReference.moduleSpecifierSymbol, this.#typeChecker);
    ...
    #isExternalModulePath(importOrExportDeclaration, moduleSpecifier) {
        ...
        const mode = specifier && ts.isStringLiteralLike(specifier)
            ? TypeScriptInternals.getModeForUsageLocation(importOrExportDeclaration.getSourceFile(), specifier, this.#program.getCompilerOptions())
            : undefined;
        const resolvedModule = TypeScriptInternals.getResolvedModule(this.#program, importOrExportDeclaration.getSourceFile(), moduleSpecifier, mode);
    ...
    fetchReferencedAstEntity(symbol, referringModuleIsExternal) {
        ...
        if (referringModuleIsExternal) {
            current = TypeScriptHelpers.followAliases(symbol, this.#typeChecker);
        }
        else {
            for (;;) {
                ...
                const currentAlias = TypeScriptInternals.getImmediateAliasedSymbol(current, this.#typeChecker);
    ...
    #fetchSpecifierAstModule(importOrExportDeclaration, exportSymbol) {
        ...
        const mode = importOrExportDeclaration.moduleSpecifier &&
            ...
            ? TypeScriptInternals.getModeForUsageLocation(importOrExportDeclaration.getSourceFile(), importOrExportDeclaration.moduleSpecifier, this.#program.getCompilerOptions())
            : undefined;
        const resolvedModule = TypeScriptInternals.getResolvedModule(this.#program, importOrExportDeclaration.getSourceFile(), moduleSpecifier, mode);
        if (resolvedModule === undefined) {
    ...
    #fetchAstImport(importSymbol, options) {
        ...
        if (!astImport) {
            ...
            if (importSymbol) {
                const followedSymbol = TypeScriptHelpers.followAliases(importSymbol, this.#typeChecker);
