...
export class Terminal {
    ...
    writeWarning(...messageParts) {
        const { parts, options: { doNotOverrideSgrCodes } } = this.#normalizeWriteParameters(messageParts);
        this.#writeSegmentsToProviders(doNotOverrideSgrCodes
            ? parts
    ...
    writeError(...messageParts) {
        const { parts, options: { doNotOverrideSgrCodes } } = this.#normalizeWriteParameters(messageParts);
        this.#writeSegmentsToProviders(doNotOverrideSgrCodes ? parts : parts.map((part) => Colorize.red(AnsiEscape.removeCodes(part))), TerminalProviderSeverity.error, false);
    }
    /**
    ...
    writeErrorLine(...messageParts) {
        const { parts, options: { doNotOverrideSgrCodes } } = this.#normalizeWriteParameters(messageParts);
        this.#writeSegmentsToProviders(doNotOverrideSgrCodes ? parts : parts.map((part) => Colorize.red(AnsiEscape.removeCodes(part))), TerminalProviderSeverity.error, true);
    }
    /**
    ...
    #writeSegmentsToProviders(segments, severity, followedByEol) {
    ...
    #normalizeWriteParameters(parameters) {
