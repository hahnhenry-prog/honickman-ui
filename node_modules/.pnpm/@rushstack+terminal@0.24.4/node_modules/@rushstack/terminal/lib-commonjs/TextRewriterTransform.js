...
class TextRewriterTransform extends TerminalTransform_1.TerminalTransform {
    ...
    #closeRewriters(states, chunkKind) {
        let text = '';
        for (let i = 0; i < states.length; ++i) {
            if (text.length > 0) {
                text = this.textRewriters[i].process(states[i], text);
    ...
    onClose() {
        this.#closeRewriters(this.#stderrStates, ITerminalChunk_1.TerminalChunkKind.Stderr);
        this.#closeRewriters(this.#stdoutStates, ITerminalChunk_1.TerminalChunkKind.Stdout);
