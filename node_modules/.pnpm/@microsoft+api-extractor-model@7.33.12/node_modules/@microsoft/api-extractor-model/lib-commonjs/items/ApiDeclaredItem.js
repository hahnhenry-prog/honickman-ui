...
class ApiDeclaredItem extends ApiDocumentedItem_1.ApiDocumentedItem {
    ...
    constructor(options) {
        ...
        this.#excerpt = new Excerpt_1.Excerpt(this.excerptTokens, { startIndex: 0, endIndex: this.excerptTokens.length });
    ...
    serializeInto(jsonObject) {
        ...
        if (this.fileUrlPath) {
            if (!(this.parent instanceof _a) || this.fileUrlPath !== this.parent.fileUrlPath) {
     ...
     */
    #buildSourceLocation() {
        const projectFolderUrl = this.getAssociatedPackage()?.projectFolderUrl;
        let fileUrlPath;
        for (let current = this; current !== undefined; current = current.parent) {
            if (current instanceof _a && current.fileUrlPath) {
