...
class CommandLineParser extends CommandLineParameterProvider_1.CommandLineParameterProvider {
    ...
    #actionsByName;
    #executed = false;
    #tabCompleteActionWasAdded = false;
    constructor(options) {
        super();
        this.#options = options;
        this.#actionsByName = new Map();
        const { toolFilename, toolDescription, toolEpilog } = options;
        this.#argumentParser = new CommandLineParserExitError_1.CustomArgumentParser({
            addHelp: true,
            prog: toolFilename,
            description: (0, escapeSprintf_1.escapeSprintf)(toolDescription),
            epilog: terminal_1.Colorize.bold((0, escapeSprintf_1.escapeSprintf)(toolEpilog ?? `For detailed help about a specific command, use: ${toolFilename} <command> -h`))
        });
    }
    /**
    ...
    async executeWithoutErrorHandlingAsync(args) {
        try {
            ...
            const data = this.#argumentParser.parseArgs(args);
            postParse();
